             ...
             for($i=0;$i<=$Parameters["Rows"];$i++)
              {
               ...
               if ( $Abscissa != NULL )
                { if ( isset($Data["Series"][$Abscissa]["Data"][$i]) ) { $Value = $Data["Series"][$Abscissa]["Data"][$i]; /* $Value = $this->scaleFormat($Data["Series"][$Abscissa]["Data"][$i],$Data["XAxisDisplay"],$Data["XAxisFormat"],$Data["XAxisUnit"]); */ } else { $Value = ""; } }
               else
                {
               ...
               if ( $this->isValidLabel($Value,$LastValue,$LabelingMethod,$ID,$LabelSkip) )
                {
                 $Bounds    = $this->drawText($XPos,$YPos+$OuterTickWidth+$YLabelOffset,$Value[0],array("Angle"=>$LabelRotation,"Align"=>$LabelAlign/*, "dontReally"=>TRUE*/));
                 $TxtBottom = $YPos+$OuterTickWidth+2+($Bounds[0]["Y"]-$Bounds[2]["Y"]);
                 $Bounds    = $this->drawText($XPos,$TxtBottom+$OuterTickWidth+$YLabelOffset,$Value[1],array("Angle"=>$LabelRotation,"Align"=>$LabelAlign/*, "dontReally"=>TRUE*/));
                 $TxtBottom += $OuterTickWidth+2+($Bounds[0]["Y"]-$Bounds[2]["Y"]);
