...
void process_key(const char *key, const char *val) {
	if(compare(key, "server")) {
		...
		conf.port = atoi(val);
	} else if(compare(key, "tcp_in_id")) {
		conf.tcp_in_id = atoi(val);
	} else if(compare(key, "tcp_out_id")) {
		conf.tcp_out_id = atoi(val);
	} else if(compare(key, "pppd_params")) {
		strcpy(conf.pppd_params, val);
	}
}
